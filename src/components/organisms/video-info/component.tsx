...
export const VideoInfo: React.FC<Props> = ({ videoData, className }) => {
  ...
  useEffect(() => {
    const val =
      localStorage.getItem("currentUser") !== null ? localStorage.getItem("currentUser") : "";
    const user = localStorage.getItem("currentUser") ? JSON.parse(val ?? "") : "";
    user?.subscriptions &&
      user.subscriptions.map((el: any) => {
        if (el.channelId === videoData.channelId) {
          setSubscribed(true);
        }
        return el;
      });
    ...
    <div className={classNames([className, "d-flex flex-column row-gap-3"])}>
      ...
      <div className="d-flex justify-content-between">
        <div className="pe-4 d-flex column-gap-3 align-items-center">
          <Anchor href={`/channel/${videoData.channelId}`}>
            <Img
              src={videoData.channelProfileSrc}
              width="48px"
              height="48px"
              alt="channel profile"
              className="rounded-5"
          ...
          <Button
            ...
            size="s2"
            borderRadius="30px"
            backgroundColor={subscribed ? Colors.background.grey : Colors.background.black}
          >
